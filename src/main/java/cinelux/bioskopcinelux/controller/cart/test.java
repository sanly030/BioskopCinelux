...
//        CartTicketItem existingItem = cartItems.stream()
//                .filter(item -> item.getJadwal().equals(jadwal))
//                .findFirst()
//                .orElse(null);
//
...
//            CartTicketItem newItem = new CartTicketItem(jadwal, quantity,
...
//
//
//    private void handleTiketSelection(JadwalTayang jadwal) {
//        try {
...
//
//
//    private void addToCart(JadwalTayang jadwal, int quantity) {
//        // Check if item already exists in cart
//        CartTicketItem existingItem = cartItems.stream()
//                .filter(item -> item.getJadwal().getId().equals(jadwal.getId()))
//                .findFirst()
//                .orElse(null);
//        if (existingItem != null) {
//            existingItem.setQuantity(existingItem.getQuantity() + quantity);
//        } else {
//            CartTicketItem newItem = new CartTicketItem(jadwal, quantity, this::updateCartDisplay, () -> {
//                cartItems.removeIf(item -> item.getJadwal().getId().equals(jadwal.getId()));
//                updateCartDisplay();
//                updateCartCount();
//                calculateTotalBayar();
//            });
...
//    //            if (currentUser == null) {
//    //                showErrorMessage("Pegawai saat ini tidak tersedia (currentUser null)");
//    //                return false;
//    //            }
//    //
//    //            // 🔹 Validasi kursi
...
//    //
//    //            // 🔹 Hitung jumlah tiket
//    //            int jumlahTiket = cartItems.stream().mapToInt(CartTicketItem::getQuantity).sum();
//    //
//    //            // 🔹 Simpan status kursi (gabungan ID)
...
//    //                    detail.setJumlah(1); // 1 kursi = 1 tiket
//    //                    detail.setSubTotal(jadwal.getHarga().doubleValue()); // bisa dihitung promo jika diperlukan
...
//    }
//
//    // 13. Method untuk enhanced total calculation display
//    private void updateTotalDisplayWithAnimation() {
//        BigDecimal subtotal = getCartSubtotal();
//        BigDecimal discount = BigDecimal.ZERO;
//
//        Promo selectedPromo = cbPromo.getSelectionModel().getSelectedItem();
//        if (selectedPromo != null) {
//            discount = calculateEnhancedPromoDiscount(subtotal, selectedPromo); // return BigDecimal
//        }
//
//        BigDecimal total = subtotal.subtract(discount);
//
//        // Animate labels (convert to double for formatting)
//        animateLabel(labelSubtotal, formatCurrency(subtotal));
//        animateLabel(labelDiskon, formatCurrency(discount));
//        animateLabel(labelTotalBayar, formatCurrency(total));
//
//        // Update colors based on discount
//        if (discount.compareTo(BigDecimal.ZERO) > 0) {
//            labelDiskon.setStyle("-fx-text-fill: #4caf50; -fx-font-weight: bold;");
//            labelTotalBayar.setStyle("-fx-text-fill: #1976d2; -fx-font-weight: bold; -fx-font-size: 18px;");
//        } else {
//            labelDiskon.setStyle("-fx-text-fill: #666666;");
//            labelTotalBayar.setStyle("-fx-text-fill: #333333; -fx-font-weight: bold; -fx-font-size: 18px;");
//        }
//
//        calculateKembalian();
//    }
//
//    // 14. Method untuk animasi label
